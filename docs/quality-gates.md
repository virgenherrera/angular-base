...
| ------------------ | --------- | ------------- | :-------: | :---------: | :--------: | :-: | :------: |
| `start`            | atomic    | serve         |    yes    |     no      |     no     | no  |    no    |
...
| 3     | test:dynamic   | `test:dynamic` (vitest — unit)                                |
| 4     | build          | **`build`** composite: `build:app`                            |
...
2. Hook it into the relevant composite: `test:static`, `build`, or configure vitest for `test:dynamic`
...
| `test:dynamic` | vitest discovers tests via config — no composite change needed                |

CI calls composites directly (`test:static`, `test:dynamic`, `build`), so new scripts propagate to CI automatically.
